...
| AI能力 | 具体服务 | 免费额度 | 用途 |
|--------|---------|---------|------|
| **OCR识别** | 百度通用文字识别(accurate) | 5万次/月 | 病历图片文字识别+坐标返回 |
| **大模型** | DeepSeek Chat API | 按量计费 | AI纠错OCR字段、病历格式整理 |
...
2. **百度智能云账号**
   ...
   - ✅ OCR服务已开通（accurate高精度版）
   - ✅ DeepSeek Chat API已接入
...
### ✅ OCR病历识别子系统（核心算法，2026-06-28完成）

**架构流水线**：`百度OCR(accurate) → OCRNormalize清洗 → FSMEngine空间匹配 → RegexEngine → Validator → LLM纠错(DeepSeek) → 本地格式整理`

| 模块 | 说明 |
|------|------|
| BaiduOCR集成(accurate) | 返回文字+坐标+置信度，支持图片预处理 |
| ImagePreprocessService | OpenCV图片预处理（灰度/二值化/降噪） |
| OCRNormalize清洗 | 坐标修正、文字拼接、字段分块 |
| FSMEngine空间匹配 | GridIndex空间索引 + 五维评分权重系统 |
| RegexEngine正则引擎 | 门诊号/性别/年龄/日期/手机号等格式字段提取 |
| Validator校验器 | 字段非空与有效性验证 |
| DeepSeekLLM纠错 | 核心字段缺失时自动触发LLM纠错(correctFields) |
| 本地格式整理(cleanFieldValues) | 性别/年龄/日期等格式统一，替代DS调用避免超时 |

**核心算法文件**：
- `MedicalOcrService.java` — OCR识别流水线整合（5层处理）
- `FSMEngine.java` — Field Spatial Matching Engine（353行）
- `FieldStrategy.java` — 17种字段策略配置（154行）
- `DeepSeekService.java` — DeepSeek API调用+Prompt构建+响应解析
- `OCRNormalize.java` — OCR文本清洗（120行）
- `Candidate.java` / `Direction.java` / `ScoringWeights.java` / `ResultStatus.java` / `FieldResult.java` — 核心数据模型

**字段映射**：日期→recordDate(出生日期智能检测)、性别→gender、年龄→age、主诉→chiefComplaint、现病史→presentHistory、既往史→pastHistory、诊断→diagnosis、处理意见→treatment(含药品信息)
...
| 日期 | 表名 | 修改内容 | 文件 |
|------|------|---------|------|
| 2026-06-28 | `medicalrecord` | 新增字段: `gender`, `age`, `past_history`, `present_history`, `chief_complaint`, `treatment` | [schema.sql](file:///e:/medical/backend/src/main/resources/schema.sql#L22-L36) |
...
| 同学A | 开发 | 用户档案管理 |
| 同学B | 开发 | 用药管理 + 服药反馈 |
| 同学C（你）| 项目负责人 | OCR病历识别子系统 + 提醒管理 + 家庭协作 + 管理员后台 |
