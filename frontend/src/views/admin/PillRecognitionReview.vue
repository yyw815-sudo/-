<template>
  <div class="pill-review-container">
    <PageHeader 
      title="药片识别审核" 
      subtitle="审核用户上传的药片图片识别结果"
      :icon="Cpu"
    ...
    <ContentCard>
      ...
      <div class="table-wrapper">
        <el-table :data="tableData" style="width: 100%" v-loading="loading" class="custom-table">
          ...
          <el-table-column prop="pillName" label="药片名称" min-width="150" />
          <el-table-column prop="confidence" label="置信度" width="120">
            <template #default="{ row }">
              <div class="confidence-cell">
                <el-progress 
                  :percentage="row.confidence ? row.confidence * 100 : 0" 
    ...
    >
      <div v-if="viewData" class="view-content">
        ...
        <div class="view-row">
          <span class="view-label">药片名称：</span>
          <span class="view-value">{{ viewData.pillName || '-' }}</span>
        </div>
        <div class="view-row">
          <span class="view-label">置信度：</span>
          <span class="view-value">{{ viewData.confidence ? (viewData.confidence * 100).toFixed(0) + '%' : '-' }}</span>
        </div>
        <div class="view-row">
          <span class="view-label">描述：</span>
          <span class="view-value">{{ viewData.pillDescription || '-' }}</span>
        </div>
        <div class="view-row">
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await aiCenterApi.getPillReviewList({
...
const handleView = async (row) => {
  try {
    const res = await aiCenterApi.getPillReviewDetail(row.id)
    if (res.code === 200) {
...
const handleSubmitReview = async () => {
  try {
    await aiCenterApi.reviewPill(reviewId.value, {
...
const handleDelete = async (row) => {
  try {
    await aiCenterApi.deletePillReview(row.id)
    loadData()
  } catch (error) {
...
const getConfidenceColor = (confidence) => {
  if (!confidence) return '#E4E7ED'
  if (confidence >= 0.8) return '#43e97b'
  if (confidence >= 0.6) return '#fa709a'
  return '#f5576c'
}
