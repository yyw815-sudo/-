...
| user_name   | varchar(50) UNIQUE | 用户名（唯一） |
...
## 九、药品测试数据（medicine表indication字段）

> 以下为适应症（indication）测试数据，用于替代方案建议时的药物分类匹配：

| medicine_id | medicine_name | indication |
|:-:|:-:|:-:|
...
> 说明：indication字段用于`MedicationPlanService.suggestAlternativeDrug()`中基于适应症关键词匹配替代药物。例如"降糖药"分类下"盐酸二甲双胍片"和"格列美脲片"冲突时，可匹配到"阿卡波糖"作为替代方案。
...
| admin_name  | varchar(50) UNIQUE | 管理员登录账号（唯一） |
...
> **【2026-06-28 修改】** 新增 `gender`, `age`, `past_history`, `present_history`, `chief_complaint`, `treatment` 六个字段，替换原 `hospital`/`department`/`doctor`/`prescription` 为更细粒度的病历信息
...
| age               | int          | 年龄         |
| past_history      | text         | 既往病史       |
| present_history   | text         | 现病史        |
